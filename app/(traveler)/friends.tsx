...
} from "../../src/hooks/use-friends-queries";
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      ...
      >
        ...
        <Pressable
          onPress={() => router.push("/(traveler)/add-friend")}
          hitSlop={8}
        >
      ...
      <View
        className="flex-row px-4 mt-3 gap-6"
        style={{ borderBottomWidth: 1, borderBottomColor: colors.border }}
      >
        {(["friends", "requests", "groups"] as Tab[]).map((t) => (
          <Pressable key={t} onPress={() => setTab(t)} className="pb-3">
            <Text
              ...
              style={{
                color: tab === t ? colors.vaykaePink : colors.mutedForeground,
              }}
            >
              {t === "friends"
        ...
        friends.isLoading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <FriendCard
                friend={item}
                onRemove={(friend) => unfriend.mutate(friend.id)}
              />
            )}
          />
        )

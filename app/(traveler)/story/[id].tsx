...
export default function StoryViewerScreen() {
  ...
  return (
    <View
      style={{ flex: 1, backgroundColor: current.backgroundColor || "#000" }}
    >
      {content}
      ...
      >
        <View className="flex-row items-center gap-3 px-4 pb-4">
          ...
          <Pressable onPress={handleLike} hitSlop={8}>
            <Heart
              size={26}
              color={isLiked ? colors.vaykaePink : "#fff"}
              fill={isLiked ? colors.vaykaePink : "transparent"}
            />
          </Pressable>
          <Pressable
            onPress={() =>
              showInDevelopmentAlert(
                "Replying to stories isn’t wired up yet.",
                undefined,
                () => setReply(""),
              )
            }

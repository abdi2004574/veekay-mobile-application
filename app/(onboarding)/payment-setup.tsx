...
const PAYMENT_METHODS = [
  {
    icon: Wallet,
    name: "PayPal",
    description: "Fast, secure payouts to your PayPal account",
  },
  {
    icon: CreditCard,
    name: "Stripe",
    description: "Direct payouts to your bank via Stripe",
  },
  {
    icon: Banknote,
    name: "Bank Account",
...
export default function PaymentSetupScreen() {
  ...
  return (
    <AuthScreenLayout>
      ...
      <Pressable
        onPress={() =>
          showInDevelopmentAlert("Adding a card isn’t available yet.")
        }
      ...
      <View style={{ gap: 10 }}>
        {PAYMENT_METHODS.map(({ icon: Icon, name, description }) => (
          <Pressable
            key={name}
            onPress={() =>
              showInDevelopmentAlert(`Connecting ${name} isn’t available yet.`)
            }
      ...
      <View className="mt-8" style={{ gap: 12 }}>
        <GradientButton
          ...
          loading={completeProfileSetup.isPending}
        >
          Skip for Now
        </GradientButton>
      </View>

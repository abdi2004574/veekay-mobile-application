...
async function getOtpForEmail(email: string): Promise<string> {
  return new Promise((resolve, reject) => {
    http
      .get(MAILHOG_URL + "/api/v2/messages", (res) => {
        let body = "";
        res.on("data", (chunk) => {
          body += chunk;
        });
        res.on("end", () => {
          try {
            ...
            const matched = messages.find((m) => {
              const to = m.To || [];
              return (
                Array.isArray(to) && to.some((t: any) => t.Email === email)
              );
            });
            if (!matched) {
            ...
            const bodyData =
              matched.Body?.data || matched.Body || matched.data || "";
            const text =
...
test.describe.serial("auth flow", () => {
  ...
  test("register and verify OTP", async ({ page }) => {
    test.setTimeout(60000);
    await page.goto(BASE_URL + "/(auth)/register?role=traveler", {
  ...
  test("login with password", async ({ page }) => {
    test.setTimeout(60000);
    await page.goto(BASE_URL + "/(auth)/login?role=traveler", {
      waitUntil: "domcontentloaded",
      timeout: 60000,
    });
...
test("app handles errors gracefully", async ({ page }) => {
  test.setTimeout(60000);
  await page.goto(BASE_URL + "/(traveler)/nonexistent-route", {
    waitUntil: "domcontentloaded",
    timeout: 60000,
  });
  await page
    .waitForSelector("text=Unmatched Route", { timeout: 10000 })
    .catch(() => {});
  const hasUnmatchedRoute =
    (await page.locator("text=Unmatched Route").count()) > 0;
  const hasNotFound =
    (await page.locator("text=Page could not be found").count()) > 0;
  const hasError =

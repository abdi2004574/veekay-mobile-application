...
export function useSendFriendRequest() {
  ...
  return useMutation({
    mutationFn: (addresseeId: string) =>
      friendsApi.sendFriendRequest(addresseeId, requireAccessToken()),
...
export function useAcceptFriendRequest() {
  ...
  return useMutation({
    mutationFn: (requestId: string) =>
      friendsApi.acceptFriendRequest(requestId, requireAccessToken()),
    onSuccess: () => {
...
export function useDeclineFriendRequest() {
  ...
  return useMutation({
    mutationFn: (requestId: string) =>
      friendsApi.declineFriendRequest(requestId, requireAccessToken()),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["friend-requests"] }),
    onError: (err) => showToast(friendlyErrorMessage(err)),
  });
...
export function useUnfriend() {
  ...
  return useMutation({
    mutationFn: (userId: string) =>
      friendsApi.unfriend(userId, requireAccessToken()),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["friends"] });
      queryClient.invalidateQueries({ queryKey: ["feed"] });

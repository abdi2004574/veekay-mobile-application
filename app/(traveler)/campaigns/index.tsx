...
export default function CampaignsScreen() {
  ...
  const completed = useMemo(
    () => items.filter((c) => c.status === "completed"),
    [items],
  );
  ...
  const confirmDelete = (campaignId: string) => {
    showAlert("Delete campaign?", "This action cannot be undone.", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Delete",
        style: "destructive",
        onPress: () => deleteCampaign.mutate(campaignId),
      },
    ]);
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-background" edges={["top"]}>
      <View style={{ flex: 1 }}>
        <Text className="text-xl font-bold text-foreground px-4 pt-3 pb-1">
          Campaigns
        </Text>

        <View
          className="flex-row gap-2 p-1 mx-4 mt-3 mb-1 rounded-2xl"
          ...
          {(
            ...
            >
              <View
                ...
                style={{
                  backgroundColor:
                    tab === t.key ? colors.vaykaePink : "transparent",
                }}
              >
                <Text
                    ...
                    >
                      ...
                      <View className="p-4">
                        ...
                        <Text
                          ...
                          style={{ color: colors.mutedForeground }}
                        >
                          {c.contributorsCount} contributors • $
                          {c.goalAmount.toLocaleString()} raised
                        </Text>
                        <GradientButton
                          onPress={() =>
    ...
    >
      ...
      <View className="p-4">
              ...
              onPress={() =>
                router.push({
                  ...
                  params: {
                    ...
                    editTripEndDate: campaign.tripEndDate
                      ? campaign.tripEndDate.slice(0, 10)
                      : "",
                    ...
                    editPhotoMediaIds: campaign.photos
                      .map((p) => p.mediaId)
                      .join(","),
                    editPhotoUrls: campaign.photos
                      .map((p) => p.url ?? "")
                      .join(","),
        ...
        <View className="flex-row gap-2 mb-3">
          <StatTile
            label="Contributors"
            value={String(campaign.contributorsCount)}
            icon={Users}
          />
          <StatTile
            label="Views"
            value={String(campaign.viewsCount)}
            icon={Eye}
          />
